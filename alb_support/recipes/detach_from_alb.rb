...
ruby_block "detach from ALB" do
  block do
    ...
    target_group_arns = node[:alb_helper][:target_group_arn]
    target_group_port = node[:alb_helper][:target_group_port]
    
    target_group_arns=[target_group_arns] if target_group_arns.instance_of? String 

    Chef::Log.info("Creating ELB client in region #{stack_region}")
    client = Aws::ElasticLoadBalancingV2::Client.new(region: stack_region, retry_limit: 10)
    target_group_arns.each { |target_group_arn|
